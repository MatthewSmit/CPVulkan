...
public:
	...
	VKAPI_ATTR VkResult VKAPI_CALL EnumerateDeviceLayerProperties(uint32_t* pPropertyCount, VkLayerProperties* pProperties) { TODO_ERROR(); }
	...
	VKAPI_ATTR VkResult VKAPI_PTR GetPresentRectangles(VkSurfaceKHR surface, uint32_t* pRectCount, VkRect2D* pRects) { TODO_ERROR(); }

#if defined(VK_KHR_display)
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayProperties(uint32_t* pPropertyCount, VkDisplayPropertiesKHR* pProperties);
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayPlaneProperties(uint32_t* pPropertyCount, VkDisplayPlanePropertiesKHR* pProperties) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayPlaneSupportedDisplays(uint32_t planeIndex, uint32_t* pDisplayCount, VkDisplayKHR* pDisplays) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayModeProperties(VkDisplayKHR display, uint32_t* pPropertyCount, VkDisplayModePropertiesKHR* pProperties) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR CreateDisplayMode(VkDisplayKHR display, const VkDisplayModeCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDisplayModeKHR* pMode) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayPlaneCapabilities(VkDisplayModeKHR mode, uint32_t planeIndex, VkDisplayPlaneCapabilitiesKHR* pCapabilities) { TODO_ERROR(); }
#endif
	...
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayProperties2(uint32_t* pPropertyCount, VkDisplayProperties2KHR* pProperties) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayPlaneProperties2(uint32_t* pPropertyCount, VkDisplayPlaneProperties2KHR* pProperties) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayModeProperties2(VkDisplayKHR display, uint32_t* pPropertyCount, VkDisplayModeProperties2KHR* pProperties) { TODO_ERROR(); }
	VKAPI_ATTR VkResult VKAPI_PTR GetDisplayPlaneCapabilities2(const VkDisplayPlaneInfo2KHR* pDisplayPlaneInfo, VkDisplayPlaneCapabilities2KHR* pCapabilities) { TODO_ERROR(); }

	VKAPI_ATTR VkResult VKAPI_PTR GetExternalImageFormatPropertiesNV(VkFormat format, VkImageType type, VkImageTiling tiling, VkImageUsageFlags usage, VkImageCreateFlags flags, VkExternalMemoryHandleTypeFlagsNV externalHandleType, VkExternalImageFormatPropertiesNV* pExternalImageFormatProperties) { TODO_ERROR(); }

	VKAPI_ATTR void VKAPI_PTR GetGeneratedCommandsPropertiesNVX(VkDeviceGeneratedCommandsFeaturesNVX* pFeatures, VkDeviceGeneratedCommandsLimitsNVX* pLimits) { TODO_ERROR(); }

	VKAPI_ATTR VkResult VKAPI_PTR ReleaseDisplay(VkDisplayKHR display) { TODO_ERROR(); }

	VKAPI_ATTR VkResult VKAPI_PTR GetSurfaceCapabilities2(VkSurfaceKHR surface, VkSurfaceCapabilities2EXT* pSurfaceCapabilities) { TODO_ERROR(); }

	VKAPI_ATTR void VKAPI_PTR GetMultisampleProperties(VkSampleCountFlagBits samples, VkMultisamplePropertiesEXT* pMultisampleProperties) { TODO_ERROR(); }

#if defined(VK_EXT_calibrated_timestamps)
	VKAPI_ATTR VkResult VKAPI_PTR GetCalibrateableTimeDomains(uint32_t* pTimeDomainCount, VkTimeDomainEXT* pTimeDomains);
#endif
	
	VKAPI_ATTR VkResult VKAPI_PTR GetCooperativeMatrixPropertiesNV(uint32_t* pPropertyCount, VkCooperativeMatrixPropertiesNV* pProperties) { TODO_ERROR(); }

	VKAPI_ATTR VkResult VKAPI_PTR GetSupportedFramebufferMixedSamplesCombinationsNV(uint32_t* pCombinationCount, VkFramebufferMixedSamplesCombinationNV* pCombinations) { TODO_ERROR(); }

	VKAPI_ATTR VkBool32 VKAPI_PTR GetWin32PresentationSupport(uint32_t queueFamilyIndex) { TODO_ERROR(); }
								  
	VKAPI_ATTR VkResult VKAPI_PTR GetSurfacePresentModes2(const VkPhysicalDeviceSurfaceInfo2KHR* pSurfaceInfo, uint32_t* pPresentModeCount, VkPresentModeKHR* pPresentModes) { TODO_ERROR(); }

#if defined(VK_KHR_xcb_surface)
    VKAPI_ATTR VkBool32 VKAPI_PTR GetPhysicalDeviceXcbPresentationSupport(uint32_t queueFamilyIndex, xcb_connection_t* connection, xcb_visualid_t visual_id) { TODO_ERROR(); }
#endif

#if defined(VK_KHR_xlib_surface)
    VKAPI_ATTR VkBool32 VKAPI_PTR GetPhysicalDeviceXlibPresentationSupport(uint32_t queueFamilyIndex, Display* dpy, VisualID visualID) { TODO_ERROR(); }
#endif

#if defined(VK_EXT_acquire_xlib_display)
    VKAPI_ATTR VkResult VKAPI_PTR AcquireXlibDisplay(Display* dpy, VkDisplayKHR display) { TODO_ERROR(); }
    VKAPI_ATTR VkResult VKAPI_PTR GetRandROutputDisplay(Display* dpy, RROutput rrOutput, VkDisplayKHR* pDisplay) { TODO_ERROR(); }
#endif
